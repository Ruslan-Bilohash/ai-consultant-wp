...
(function () {
    ...
    chatContainer.id = 'ai-consultant-chat';
    ...
    chatContainer.innerHTML = `
        <div style="background:${s.header_gradient || 'linear-gradient(135deg,#00f5ff 0%,#0099ff 100%)'};color:#000;padding:20px 24px;display:flex;align-items:center;justify-content:space-between;font-weight:800;font-size:20px;border-radius:28px 28px 0 0;">
            <div style="display:flex;align-items:center;gap:14px;">
                ...
                <div>
                    <div>${s.chat_title || 'AI Consultant'}</div>
                    <div style="font-size:13.8px;opacity:0.9;">${s.chat_subtitle || 'Profesionalus valymo konsultantas'}</div>
                </div>
            </div>
        ...
        <div style="padding:18px 22px;border-top:1px solid rgba(0,245,255,0.25);background:rgba(15,15,45,0.98);display:flex;gap:12px;">
            <input id="ai-input" type="text" placeholder="Напишіть повідомлення..." 
        ...
        <div style="padding:10px 20px;text-align:center;font-size:12.5px;color:rgba(255,255,255,0.45);background:rgba(0,0,0,0.3);border-top:1px solid rgba(0,245,255,0.2);">
            Powered by <a href="https://bilohash.com/ai" target="_blank" style="color:#00f5ff;text-decoration:none;">AI Consultant</a>
        </div>
    `;
    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (data.reply) {
                ...
                addMessage('Помилка: ' + (data.error || data.message), 'bot');
            } else {
                addMessage('Не вдалося отримати відповідь від бота.', 'bot');
            }
        } catch (err) {
            if (loadingEl && loadingEl.parentNode) loadingEl.remove();
            addMessage('Вибачте, проблема зі зв’язком. Спробуйте ще раз.', 'bot');
        ...
        setTimeout(() => {
            if (chatContainer.style.display !== 'flex') {
                ...
                if (messagesDiv.children.length === 0) {
                    setTimeout(() => {
                        addMessage(s.welcome_text || 'Привіт! Чим можу допомогти сьогодні?', 'bot');
                    }, 400);
                }
            }
    ...
    console.log('✅ AI Consultant Chat v2.3.4 — успішно ініціалізовано');
