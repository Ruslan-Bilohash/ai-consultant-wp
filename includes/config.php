<?php
if (!defined('ABSPATH')) {
    exit;
}

$settings = get_option('ai_consultant_wp_settings', ai_consultant_wp_default_settings());

define('XAI_API_KEY', $settings['xai_api_key'] ?? '');
define('TELEGRAM_TOKEN', $settings['telegram_token'] ?? '');
define('YOUR_TELEGRAM_CHAT_ID', (int)($settings['telegram_chat_id'] ?? 0));
define('GROK_MODEL', 'grok-3-mini');

define('CONVERSATIONS_DIR', AI_CONSULTANT_WP_PATH . 'conversations');
define('LOG_DIR', AI_CONSULTANT_WP_PATH . 'logs');

if (!is_dir(CONVERSATIONS_DIR)) {
    wp_mkdir_p(CONVERSATIONS_DIR);
}

if (!is_dir(LOG_DIR)) {
    wp_mkdir_p(LOG_DIR);
}

// Захист директорій від прямого доступу через HTTP
foreach ([CONVERSATIONS_DIR, LOG_DIR] as $_ai_dir) {
    $_ai_htaccess = $_ai_dir . '/.htaccess';
    if (is_dir($_ai_dir) && !file_exists($_ai_htaccess)) {
        file_put_contents($_ai_htaccess, "deny from all\n"); // phpcs:ignore WordPress.WP.AlternativeFunctions
    }
}
unset($_ai_dir, $_ai_htaccess);
